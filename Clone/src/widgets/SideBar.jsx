...
const navTop = [
  { id: "inbox", label: "Inbox", icon: <ArticleOutlinedIcon />, badge: 0 },
  { id: "radar", label: "Insights", icon: <RadarOutlinedIcon /> },
  { id: "chat", label: "Messages", icon: <ChatBubbleOutlineOutlinedIcon /> },
  { id: "team", label: "Team", icon: <Groups2OutlinedIcon /> },
];

export default function CompactSidebar({clicked,setClicked}) {
      ...
      <Box className="h-screen w-full flex flex-col justify-between bg-[#1f2225] text-zinc-400">
        <Box className="flex flex-col items-center gap-3 pt-3">
          {navTop.map((item) => {
            ...
            const iconBtn = (
              <IconButton
                onClick={() => {setActive(item.id);setClicked(!clicked)}}
                className={[
                  "w-12 h-12",
                  "rounded-2xl",
